...
var init = function init(selector) {
    selector.querySelector('.songPreviewPlayButton').addEventListener('click', function () {
        playPreview(selector);
    });
    selector.querySelector('.songPreviewStopButton').addEventListener('click', function () {
        stopPreview(selector);
    });
};

var playPreview = function playPreview(songDiv) {
    console.log(songDiv);
    console.log(songDiv.querySelector('.songPreviewAudio'));
    var audios = document.querySelectorAll('.songPreviewDiv');
...
var stopPreview = function stopPreview(songDiv) {
...
var init = function init(selector) {
    selector.querySelector('.product-button').addEventListener('click', function () {
        checkPromos(selector);
    });
    selector.querySelector('.checkout-cart-item-buttons').addEventListener('click', function () {
        removePromos(selector);
    });
};

var checkPromos = function checkPromos(selector) {
    var button = selector.querySelector('.product-button');
    var code = document.getElementById(button.getAttribute('data-inputid')).value;

    var viewMode = selector.getAttribute('data-viewmode');

    var string = 'checkout-promos-message' + viewMode;
    document.getElementById(string).parentElement.classList.add('hidden');
    document.getElementById(string).innerHTML = '';

    if (code) {
        var url = window.location.href.replace('checkout', 'addpromo') + '?code=' + code;
        console.log(url);
        __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(url, function (data, status) {
            var message = data[0];
            if (message.type == 'Success') {
                document.getElementById('shipping' + viewMode).innerHTML = '<p><b>Shipping: </b><span class="cart_total-mobile">$' + parseFloat(data[2]).toFixed(2) + '</span></p>';
                var _string = 'checkout-promos-div' + viewMode;
                document.getElementById(_string).classList.add('hidden');
                _string = 'checkout-promos-applied' + viewMode;
                document.getElementById(_string).children[1].innerHTML = message.message;
                document.getElementById(_string).classList.remove('hidden');
                updateCart(data[1], data[2], viewMode);
            } else {
                var _string2 = 'checkout-promos-message' + viewMode;
                document.getElementById(_string2).parentElement.classList.remove('hidden');
                document.getElementById(_string2).classList.add('promo-message-' + message.type);
                document.getElementById(_string2).innerHTML = message.message;
            }
        });
    }
};

var removePromos = function removePromos(selector) {
    var viewMode = selector.getAttribute('data-viewmode');
    var url = window.location.href.replace('checkout', 'removepromo');
    var button = selector.querySelector('.product-button');
    var code = document.getElementById(button.getAttribute('data-inputid')).value;
    __WEBPACK_IMPORTED_MODULE_0_jquery___default.a.get(url, function (data, status) {
        var message = data[0];
        ...
        if (message.type == 'Success') {
            ...
            var string = 'checkout-promos-div' + viewMode;
...
var updateCart = function updateCart(cart_items, shipping, viewMode) {
    ...
    cart_items.forEach(function (element) {
        var cart_row = document.getElementById('cart_item-' + element.cart_item_id + viewMode);

        element.product_price = element.price + element.discount;

        var price_html = '<p>$' + (element.product_price * element.quantity).toFixed(2) + '</p>';
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_jquery__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_jquery___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_0_jquery__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__paypal_paypal_js__ = __webpack_require__(58);



var init = function init(selector) {

  Object(__WEBPACK_IMPORTED_MODULE_1__paypal_paypal_js__["a" /* loadScript */])({ "client-id": 'AUEml5vTR3kDWwv3LjQkgLA1KP6ISueJoXxyqAps-dZd0MwtBbkhN1qwXNjyd89JizzAmJSVlIctFtl_' }).then(function (paypal) {
    paypal.Buttons({
...
"use strict";
/* unused harmony export loadCustomScript */
